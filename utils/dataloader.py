    ...
    def __getitem__(self, i):
        ...
        if None in best: # counldn't find a good window
            ...
            if img_a.shape[:2][::-1] != output_size_a:
                ...
                img_a = np.asarray(Image.fromarray(img_a).resize(output_size_a, Image.Resampling.LANCZOS))
            ...
            if img_b.shape[:2][::-1] != output_size_b:
                ...
                img_b = np.asarray(Image.fromarray(img_b).resize(output_size_b, Image.Resampling.LANCZOS))
                aflow *= [[[sx]], [[sy]]]
...
def get_loader(loader, iscuda, threads, batch_size=1, shuffle=True):
...
if __name__ == '__main__':
    ...
    from datasets.viz import show_flow
    for data in loader:
        aflow = data['aflow']
        H, W = aflow.shape[-2:]
