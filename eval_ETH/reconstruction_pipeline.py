...
def import_features_and_match(args, paths, device):
    ...
    for image_name, image_id in tqdm(images.items(), total=len(images.items())):
        feature_path = paths.image_path/'{}.{}'.format(image_name, args.method_postfix)
    ...
    # custom match
    matcher = getattr(cms, args.matcher)
    image_names = list(images.keys())
    image_pairs = []
    image_pair_ids = set()
    for idx_total, image_name1 in enumerate(tqdm(image_names[:-1])):
        feature_path1 = paths.image_path/'{}.{}'.format(image_name1, args.method_postfix)
        ...
        for idx_sub, image_name2 in enumerate(bar):
            ...
            feature_path2 = paths.image_path/'{}.{}'.format(image_name2, args.method_postfix)
            descriptors2 = np.load(feature_path2)['descriptors']
            descriptors2 = torch.from_numpy(descriptors2).to(device)

            matches = matcher(descriptors1, descriptors2, args.matcher_ratio)
...
def reconstruct(args, paths):
    database_path = paths.database_path
    image_path = paths.image_path
    sparse_path = paths.sparse_path
    dense_path = paths.dense_path
...
def parse_args():
    ...
    parser.add_argument("--dataset_path", type=str, default='ETH_dataset', help="Path to the dataset, e.g., path/to/Fountain")

    #Madrid_Metropolis South-Building Gendarmenmarkt Tower_of_London
    parser.add_argument("--subfolder", type=str, default='Gendarmenmarkt', help="Path to the dataset")
    parser.add_argument("--save_folder", type=str, default='Gendarmenmarkt_08_50000_nn_095', help="Path to the dataset")
    parser.add_argument("--colmap_path", type=str, default='/data3/lizy/colmap/colmap-3.8/build/src/exe', help="Path to the COLMAP executable folder, e.g., path/to/colmap/build/src/exe")
    # parser.add_argument("--features_path", required=True, help="Path to the features folder, e.g., path/to/feature")
    parser.add_argument("--method_postfix", type=str, default='G_D_08_50000', help="the postfix of the method")
    parser.add_argument("--matcher", type=str, default='mutual_nn_ratio_matcher', help="the matcher") # mutual_nn_ratio_matcher mutual_nn_matcher
    parser.add_argument("--matcher_ratio", type=int, default=0.95, help="the matcher ratio")
...
def main():
    ...
    paths.colmap_path = Path(args.colmap_path)/'colmap'
    paths.dataset_path = Path(args.dataset_path)/'{}'.format(args.subfolder)
    paths.image_path = paths.dataset_path/"images"

    paths.database_path = Path(save_path)/'{}_{}.db'.format(args.subfolder, args.method_postfix)
    paths.match_list_path = Path(save_path)/'image_pairs_{}.txt'.format(args.method_postfix)
    paths.sparse_path = Path(save_path)/"{}_sparse".format(args.subfolder)
    paths.dense_path = Path(save_path)/"{}_dense".format(args.subfolder)
    paths.result_path = Path(save_path)/'res_{}_{}.txt'.format(args.subfolder, args.method_postfix)

    # print(paths.match_list_path)
    if paths.database_path.exists():
        raise FileExistsError('The {} database already exists for method \
            {}.'.format(args.subfolder, args.method_postfix))
    ...
    matching_stats = import_features_and_match(args, paths, device)
    reconstruction_stats = reconstruct(args, paths)
